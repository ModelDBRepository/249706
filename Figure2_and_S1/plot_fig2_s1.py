...
def f_plot_weight_evo(w_evo,name,wmax_exc,wmin_exc): 
    ...
    plt.text(0.6, -21, r'$\star$', fontsize=2.2*fs)
    plt.text(3.5, -23, r'$\ddag$', fontsize=1.5*fs)
    plt.xticks([0,10,15,20,25,30], fontsize=fs)
    plt.yticks(fontsize=fs)
    ax = plt.gca()
    ax.yaxis.set_label_coords(-.15,.5)
    plt.annotate("",
                xy=(-7, 0), xycoords='data',
                xytext=(-7, 250), textcoords='data',
                arrowprops=dict(arrowstyle="<->",
                                connectionstyle="arc3",linewidth=lw),annotation_clip=False)
    plt.text(-12, -10, 'Contra', fontsize=fs)
    plt.text(-10, 255, 'Ipsi', fontsize=fs)
    plt.savefig('img/Fig2_W_evo_'+name+'.eps',bbox_inches='tight')
...
plt.text(152.5, 11, r'$\theta_L$', fontsize=fs)
plt.ylim(y_lim)
...
ax.spines['top'].set_visible(False)
ax.xaxis.set_ticks_position('bottom')
plt.savefig('img/Fig2_prepost'+'.eps',bbox_inches='tight')
...
plt.savefig('img/Fig2_odi'+'.eps',bbox_inches='tight')
