...
###### Parameters
#####################################
store_name = 'dat_totalSim_evo'
store_osi = 'dat_totalSim_OSI'
...
for ii in range(NrE_L3):
    arr = []
    for _ in range(nr_Ori):
        a = (np.random.permutation(np.arange(NrE_L4))<Nr4to3)*1
        arr = arr+a.tolist()
...
epsilon = 1e-8
odi_4 = (rvv2[:,1]-rvv2[:,0])/(rvv2[:,1]+rvv2[:,0]+epsilon)
odi_23 = (rvv_d3[:,1]-rvv_d3[:,0])/(rvv_d3[:,1]+rvv_d3[:,0]+epsilon)


plt.figure()
plt.hist(odi_4,bins=bns,range=(-1,1),facecolor="None",edgecolor='k',linewidth=lw)
...
plt.annotate("",
            xy=(-.8, 240), xycoords='data',
            xytext=(.8, 240), textcoords='data',
            arrowprops=dict(arrowstyle="<->",
                            connectionstyle="arc3",linewidth=lw))
plt.text(-.9, 210, 'Ipsi', fontsize=fs)
plt.text(.65, 210, 'Contra', fontsize=fs)
plt.xlabel('ODI [a.u.]',fontsize=fs)
plt.ylabel('Number of neurons [a.u.]',fontsize=fs)
plt.title('ODI distribution in L IV',fontsize=fs)
plt.savefig('img/Fig4_odi_distri_L4.eps',bbox_inches='tight')


plt.figure()
plt.hist(odi_23,bins=bns,range=(-1,1),facecolor="None",edgecolor='k',linewidth=lw)
plt.xticks([-1,-0.5,0,0.5,1],fontsize=fs)
plt.yticks(fontsize=fs)
plt.ylim([0,60])
ax = plt.gca()
...
ax.xaxis.set_ticks_position('bottom')
plt.annotate("",
            xy=(-.8, 50), xycoords='data',
            xytext=(.8, 50), textcoords='data',
            arrowprops=dict(arrowstyle="<->",
                            connectionstyle="arc3",linewidth=lw))
plt.text(-.9, 45, 'Ipsi', fontsize=fs)
plt.text(.65, 45, 'Contra', fontsize=fs)
plt.xlabel('ODI [a.u.]',fontsize=fs)
plt.ylabel('Number of neurons [a.u.]',fontsize=fs)
plt.title('ODI distribution in L II/III',fontsize=fs)
plt.savefig('img/Fig4_odi_distri_L23.eps',bbox_inches='tight')
